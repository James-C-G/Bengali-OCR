...
def build_model():
    pickle_in = open(MATRIVASHA_DIR + "male/x.pickle", "rb")
    x = pickle.load(pickle_in)

    pickle_in = open(MATRIVASHA_DIR + "male/y.pickle", "rb")
    ...
    # Final set of convolutions with Leaky ReLU's
    model.add(layers.Conv2D(filters=64, kernel_size=3, padding='same', activation='relu'))
    model.add(layers.LeakyReLU(alpha=0.3))
    model.add(layers.Conv2D(filters=64, kernel_size=3, padding='same', activation='relu'))
    model.add(layers.LeakyReLU(alpha=0.3))
    ...
    # Output layer
    model.add(layers.Dense(120, activation='softmax'))  # 120 compound chars for matrivasha
    ...
    tf.keras.models.save_model(model, "models/matrivash_model.h5")
